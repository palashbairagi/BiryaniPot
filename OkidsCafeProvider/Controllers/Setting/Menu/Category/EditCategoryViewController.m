...
-(void)updateRecord
{
    ...
    {
        DebugLog(@"EditCategoryViewController [updateProfile]: %@ %@",e.name, e.reason);
        [Validation showSimpleAlertOnViewController:self withTitle:@"Error" andMessage:@"Unable to Process"];
        [overlayView dismiss:YES];
    }
    @finally
...
-(void) initComponents
{
    ...
    _name.text = _category.categoryName;
    
    NSData * imgData = [NSData dataWithContentsOfURL:[NSURL URLWithString:_category.imageURL]];
    ...
    if([[NSString stringWithFormat:@"%@", _category.isNonVeg] caseInsensitiveCompare:@"0"] == NSOrderedSame)
    {
